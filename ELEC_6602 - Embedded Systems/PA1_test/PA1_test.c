...
void EXTIPA1() iv IVT_INT_EXTI1 ics ICS_AUTO {
    // EXTI_PR.PA0 = 1;     // Clear interrupt pending regsiter
        EXTI_PR |= 1 << 1;      // PA1 == PR1 bit
    // GPIOE_ODR = ~GPIOE_ODR;
    // GPIO_TEST->b13 = 0;
    // *GPIO_TEST = ~(*GPIO_TEST);
            *GPIO_TEST = 0x0000;
...
void main() {
    ...
    // RCC_APB1RSTR = 0x00000001;

    // AFIO_MAPR    = 0x00000308;                 // Bit[2] USART1 Remap 0: No remap remap timer2 stuff
    AFIO_MAPR |= 1 << 3;
    AFIO_MAPR |= 1 << 8;
    AFIO_MAPR |= 1 << 9;
    ...
    GPIOA_CRL = 0x44444444;                   // PA1 input CNF=input-floating b01; MODE: input b00 = 0x0100;
    ...
    // AFIO_EXTICR1 |= 0x00000000;                // PA1 as External interrupt; Pin1/EXTI1[3:0] PortA=b0000
    // AFIO_EXTICR2 |= 0x00000100;                // PB6 as Enternal interrupt; Pin6/EXTI6[3:0] PortB=b0001 
    // EXTI_RTSR    |= 0x00000042;                // Set Edge trigger register for interrupt on rising edge for PA1 and PB6                  
    //                                            // Mask to enable TR6 and TR1 (EXTI6 and EXTI1) b1=rising trigger enabled

    // EXTI_IMR     |= 0x00000042;                // Set interrupt request from TR6 and TR1 to not masked (cpu CANNOT ignore it)
    ...
    //  */
    // NVIC_ISER0 |= 1 << 7;                     // Enable NVIC interrupt set enable registers for EXTI1 line zero (PA1)
    ...
    // // 7 % 4 = 1 R3; IPRx=1; offset=3=bit[24]
